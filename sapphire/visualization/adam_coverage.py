...
def predicted_vs_true_parameters(constraint_results,constraint_labels,constraint_colors,par_labels,output_path,savefig=False):
...
def summaries(constraint_results,constraint_labels,constraint_colors,constraint_flags,constraint_corrs,par_labels,output_path,savefig=False):
...
##### TO DO: move read_mocks outside so can call things
def plot(output_path,mocknums,constraint_flags,constraint_labels,constraint_colors,par_labels,suffix='',savefig=False):
    ...
    ### fraction of mocks where true minimum is actually a minimum according to hessian/fisher 
    good100 = np.array([1 if np.isfinite(constraint_results[0][:,3][i]).all() else 0 for i in range(constraint_results[0][:,3].shape[0])])
    good110 = np.array([1 if np.isfinite(constraint_results[1][:,3][i]).all() else 0 for i in range(constraint_results[1][:,3].shape[0])])
    good111 = np.array([1 if np.isfinite(constraint_results[2][:,3][i]).all() else 0 for i in range(constraint_results[2][:,3].shape[0])])
    # good111_err = np.array([1 if np.isfinite(out111[:,3][i]).all() else 0 for i in range(out111[:,3].shape[0])])
    
    print('fraction of mocks with true minimum',
          np.sum(good100)/len(good100), 
          np.sum(good110)/len(good110), 
          np.sum(good111)/len(good111),flush=True)
          # np.sum(good111_err)/len(good111_err),flush=True)

    
    ### save predicted vs true plot
    predicted_vs_true_parameters(constraint_results,constraint_labels,constraint_colors,par_labels,output_path,savefig)

    ### save summary plot
    summaries(constraint_results,constraint_labels,constraint_colors,constraint_flags,constraint_corrs,par_labels,output_path,savefig)
